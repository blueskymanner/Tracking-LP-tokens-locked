...
module.exports = async function UnicryptBSC() {
  ...
  cron.schedule('* * * * *', async () => {
    ...
    if (prev_TotalNums == total_tokenNums) {
      console.log("very equal to unicryptBSC."); 
      return;
    }
    
    else {
      console.log("very different to unicryptBSC.");
      prev_TotalNums = total_tokenNums;
      let tokenData0;
      let tokenData1;
      let datainfo0;
      let datainfo1;
      ...
      const tokenAddrsArr = await unicryptBSCPortal.methods.getLockedTokenAtIndex(prev_TotalNums - 1).call();
